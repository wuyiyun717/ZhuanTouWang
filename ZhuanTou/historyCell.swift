...
class historyCell: UITableViewCell{
    ...
    func addRest(float: Float!){
        self.rest = UILabel()
        self.rest.frame = CGRectMake(UIScreen.mainScreen().applicationFrame.width / 4 * 3, 0, UIScreen.mainScreen().applicationFrame.width / 4, 30)
        self.rest.setTranslatesAutoresizingMaskIntoConstraints(false)
        self.rest.text = String(format:"%.2f",float)
        self.rest.layer.borderColor = UIColor.lightGrayColor().CGColor
        self.rest.layer.borderWidth = 0.5
        self.rest.font = UIFont.systemFontOfSize(14)
